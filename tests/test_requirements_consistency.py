...
Nothing installs requirements.txt — `tests.yml` and `update-vault.yml` both
...
exactly what happened here: three open dependabot PRs (#53, #57, #58) were all
inert, and two constraints — `anthropic>=0.117.0` against a locked `0.116.0`,
and `google-auth>=2.56.0` against `2.55.1` — had *already* drifted past their
pins on `main`, from earlier bumps merged without a recompile.
...
def test_locked_versions_satisfy_declared_constraints():
    ...
    assert not violations, (
        ...
        + "\nRecompile with `uv pip compile requirements.txt -o "
          "requirements.lock --python-version 3.12`"
    )
